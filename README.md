...
启动后访问 `http://localhost:3000`（或 Docker 部署时的 `http://<IP>:18099`），首次进入需要登录。
...
使用插件 SDK CLI（基于 `@dian/sdk` 的开发者模式）：
...
插件在运行时可以使用 `@dian/storage` 提供的 `PluginStore` 接口，自动获得独立的 SQLite 表空间。
