...
export function AppSidebar({ active, onNavigate, pluginNavItems = [] }: AppSidebarProps) {
  return (
    <Sidebar collapsible="icon">
      <SidebarHeader className="gap-3 pb-1">
        <div className="flex items-center gap-2.5 px-1 py-1.5">
          ...
          <div className="grid flex-1 text-left leading-tight group-data-[collapsible=icon]:hidden">
            <span className="truncate text-[13px] font-bold tracking-tight text-gray-800">
              Dian
            </span>
            <span className="truncate text-[10px] font-medium tracking-wider text-gray-400">
              OneBot Framework
            </span>
          </div>
        </div>
        ...
        {navGroups.map((group, gi) => {
          ...
          return (
            <SidebarGroup key={group.label ?? `g-${gi}`} className="py-1">
              {group.label && (
                <SidebarGroupLabel className="mt-2 px-2 text-[10px] font-semibold uppercase tracking-[0.1em] text-gray-400/70">
                  {group.label}
                </SidebarGroupLabel>
              )}
              <SidebarGroupContent>
                    ...
                    return (
                      <SidebarMenuItem key={item.key}>
                        <SidebarMenuButton
                          ...
                          className={cn(
                            "group/item relative gap-2.5 rounded-xl px-2.5 py-2 text-[13px] font-medium transition-all duration-200",
                            isActive
                              ? "!bg-violet-50/80 !text-violet-700 shadow-sm shadow-violet-100/50"
                              : "text-gray-500 hover:bg-gray-50/80 hover:text-gray-700",
                          )}
                        >
                          {isActive && (
                            <span className="absolute left-0 top-1/2 -translate-y-1/2 size-1 rounded-full bg-violet-400 opacity-80" />
                          )}
                          <Icon
      ...
      <SidebarFooter className="mt-auto border-t border-gray-200/40 py-3 group-data-[collapsible=icon]:hidden">
        <div className="flex flex-col gap-0.5 px-2">
          <a
            href="https://github.com/FinalDevHQ/Dian"
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center gap-2.5 rounded-xl px-2.5 py-2 text-[12px] text-gray-400 transition-all duration-200 hover:bg-gray-50/80 hover:text-gray-600"
          >
